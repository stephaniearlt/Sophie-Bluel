...
function createCategoryButtons(categories) {
    ...
    allProjectsButton.addEventListener('click', () => filterProjectsByCategory(''));
    ...
    portfolioSection.querySelector('h2').insertAdjacentElement('afterend', categoryButtonsContainer);
}
...
function filterProjectsByCategory(categoryId) {
    ...
    if (categoryId !== '') {
        const clickedButton = document.querySelector(`.category-button[data-category-id="${categoryId}"]`);
        clickedButton.classList.add('clicked');
    } else {
        // Si categoryId est vide (bouton "Tous" cliqué), ajouter la classe 'clicked' au bouton "Tous"
        const allProjectsButton = document.querySelector(`.category-button[data-category-id=""]`);
...
function fetchCategoriesAndCreateButtons() {
    fetch(categoriesUrl)
        ...
        .then(categories => {
            ...
            // Suppression des travaux pré-existants
            gallery.innerHTML = '';
        })
        .catch(error => {
